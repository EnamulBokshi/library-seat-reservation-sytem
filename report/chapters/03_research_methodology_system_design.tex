...
FR-03 & Dynamic QR Verification & Tokenized QR code check-in/check-out with automatic no-show cancellation after grace periods. & High \\
...
FR-09 & Lending Policy Control & Global configuration of loan durations, max borrow limits, grace periods, and overdue penalties. & Medium \\
FR-10 & Reporting \& Analytics & Statistical analytics covering seat occupancy rates, popular books, and active circulation trends. & Low \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Digital PDF Repository}: Provides secured cloud/local storage of digitized literature with embedded PDF viewers and download endpoints.
...
\begin{itemize}
    ...
    \item \textbf{Schedule \& Booking}: Manages discrete daily time slots (\texttt{morning}, \texttt{noon}, \texttt{afternoon}, \texttt{evening}) and seat reservations with composite unique key \texttt{[seatId, scheduleId]} to prevent double-booking.
    \item \textbf{Book \& Category}: Houses bibliographic metadata (ISBN, title, author, edition, description, PDF URL) linked to specific thematic categories.
    ...
    \item \textbf{BorrowRecord \& BorrowRenewal}: Tracks lending lifecycles, due dates, return timestamps, renewal counters, and staff approval audit trails.
\end{itemize}
