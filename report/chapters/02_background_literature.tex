...
When scanned at the library entry station or zone desk, the server validates the token against the active schedule window $W = [t_{\text{start}} - \Delta t_{\text{early}}, t_{\text{start}} + \Delta t_{\text{grace}}]$. If check-in fails within the grace period $\Delta t_{\text{grace}}$, the seat status transitions automatically to \texttt{no\_show}, immediately freeing the resource.
