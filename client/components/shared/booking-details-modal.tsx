...
export function BookingDetailsModal({ booking, onClose, onCancel }: BookingDetailsModalProps) {
  ...
  const tableName =
    booking.bookingSeats && booking.bookingSeats.length > 0
      ? booking.bookingSeats[0].seat?.tableNumber
      : booking.seat?.tableNumber;

  const totalSeatsCount = booking.bookingSeats?.length || (booking.seatId ? 1 : 1);

  const handleDownloadQr = () => {
    if (!booking.qrCodeImage) return;
    const link = document.createElement("a");
    link.href = booking.qrCodeImage;
    link.download = `library-pass-${booking.seat?.seatNumber ?? "seats"}-${booking.id.slice(0, 6)}.png`;
      ...
      <div className="flex min-h-full items-center justify-center">
        <div className="pulse-card relative w-full max-w-xl p-6 sm:p-7 shadow-2xl my-auto">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mt-1">
              <div>
                <h2 className="text-xl font-extrabold text-slate-900">
                  {totalSeatsCount > 1
                    ? `Group Pass (${totalSeatsCount} Seats: ${allSeatNumbers})`
                    : `Seat ${allSeatNumbers} Pass`}
                </h2>
                <p className="text-xs text-slate-400 font-mono mt-0.5">ID: #{booking.id}</p>
              </div>
              <span
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-medium">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="kicker-label mb-1">
                  {totalSeatsCount > 1 ? `RESERVED SEATS (${totalSeatsCount})` : "SEAT & ZONE"}
                </p>
                <p className="font-extrabold text-slate-900 text-base">{allSeatNumbers}</p>
                <p className="text-slate-600 font-semibold mt-0.5">
                  {zoneName} {tableName ? `• ${tableName}` : ""}
                </p>
              </div>
                ...
                {booking.schedule ? (
                  <>
                    ...
                    <p className="text-slate-600 font-semibold capitalize mt-0.5">
                      ...
                      {SLOT_TIMES[booking.schedule.slot] ?? "Scheduled"})
                    </p>
                  </>
                ) : (
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 mt-1">
                  <div>
                    <p className="font-bold text-slate-900 text-sm">
                      {booking.user?.name ?? "Current User"}
                    </p>
          ...
          <div className="mt-6 flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button onClick={onClose} className="pulse-button-primary text-xs py-2 px-5">
              Close
            </button>
          </div>
        </div>
