#!/usr/bin/env python3
"""
Universal LaTeX to PDF / DOCX Compiler
======================================
Compiles LaTeX source files (.tex) to PDF (.pdf) or Word (.docx).
Works standalone in any folder or as the master compiler at the root of the project.

Usage:
  python3 compile.py                       # Auto-detects and compiles target (paper/report.tex or main.tex)
  ...
  python3 compile.py template              # Compiles template/main.tex
  python3 compile.py paper                 # Compiles paper/report.tex
  python3 compile.py path/to/file.tex      # Compiles specific .tex file
"""
...
def create_image_placeholder(filepath: Path):
    ...
    try:
        ...
        # Fallback to minimal binary file
...
def resolve_target_file(target: str) -> Path:
    ...
    # Check if target is a keyword: paper or template
    if target.lower() in ["paper", "report"]:
        cand = Path("paper/report.tex").resolve()
        if cand.exists():
            return cand
    elif target.lower() in ["template", "main"]:
        cand = Path("template/main.tex").resolve()
        if cand.exists():
            return cand
            
    # Auto-detection based on current directory
    if Path("report.tex").exists():
        return Path("report.tex").resolve()
    if Path("main.tex").exists():
        return Path("main.tex").resolve()
    if Path("paper/report.tex").exists():
        return Path("paper/report.tex").resolve()
    if Path("template/main.tex").exists():
        return Path("template/main.tex").resolve()

    return p.resolve()

def main():
    parser = argparse.ArgumentParser(description="Universal LaTeX to PDF / DOCX Compiler")
    parser.add_argument("target", nargs="?", default="", help="Target .tex file or keyword ('paper', 'template')")
    ...
    if not target_path.exists():
        ...
        print("     • python3 compile.py paper     (compiles paper/report.tex)")
        print("     • python3 compile.py template  (compiles template/main.tex)")
        sys.exit(1)
