...
import { BookingStatus, TableType } from "../../generated/enums";
import {
    ICreateSeatPayload,
    ICreateTableClusterPayload,
    IBulkCreateTablesPayload,
    IUpdateSeatPayload,
} from "./seat.interface";
...
const createSeat = async (payload: ICreateSeatPayload) => {
    ...
    const seat = await prisma.seat.create({
        data: {
            ...
            tableType: payload.tableType || zone.defaultTableType || TableType.individual_cubicle,
 ...
 */
const createTableCluster = async (payload: ICreateTableClusterPayload) => {
    const zone = await prisma.zone.findUnique({
        where: { id: payload.zoneId },
    });
    ...
    if (existing.length > 0) {
        throw new AppError(
            status.CONFLICT,
            `Seats for table "${payload.tableNumber}" already exist or conflict with existing seat numbers (${existing.map(s => s.seatNumber).join(", ")}).`
        );
    }

    const createdSeats = await prisma.$transaction(
        seatData.map((data) => prisma.seat.create({ data }))
    );
 ...
 */
const bulkCreateTables = async (payload: IBulkCreateTablesPayload) => {
    const zone = await prisma.zone.findUnique({
        where: { id: payload.zoneId },
    });
    ...
    const prefix = payload.tablePrefix?.trim() || (payload.tableType === TableType.circle_table ? "T" : "M");
    ...
    if (validToCreate.length === 0) {
        throw new AppError(status.CONFLICT, "All generated seats already exist in this zone.");
    }

    const createdSeats = await prisma.$transaction(
        validToCreate.map((data) => prisma.seat.create({ data }))
    );
 ...
 */
const deleteTable = async (zoneId: string, tableNumber: string) => {
    const seats = await prisma.seat.findMany({
        where: { zoneId, tableNumber },
    });

    if (seats.length === 0) {
        throw new AppError(status.NOT_FOUND, `No seats found for table ${tableNumber}`);
    }
