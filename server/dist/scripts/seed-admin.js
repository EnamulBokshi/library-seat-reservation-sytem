...
async function seedZones() {
    ...
    for (const zone of zones) {
        const existing = await prisma_1.default.zone.findUnique({ where: { name: zone.name } });
        if (existing) {
            console.log(`  ℹ️  Zone already exists: ${zone.name}`);
            continue;
        }
        const created = await prisma_1.default.zone.create({ data: zone });
...
async function seedSeats() {
    ...
    for (const zone of zones) {
        const existingSeats = await prisma_1.default.seat.count({ where: { zoneId: zone.id } });
        if (existingSeats > 0) {
        ...
        await prisma_1.default.seat.createMany({ data: seats });
